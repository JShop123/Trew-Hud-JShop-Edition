...
CreateThread(function()
	...
	while true do
		...
		if Config.ui.showOxygen  then
			showPlayerStatus = (showPlayerStatus+1)

			playerStatus['status'][showPlayerStatus] = {
				name = 'oxygen',
				value = GetPlayerUnderwaterTimeRemaining(Playerid) * 10,
			}
		end
	   ...
	   local vip
        ...
        for i=1, #ESX.PlayerData.accounts, 1 do
          ...
          elseif ESX.PlayerData.accounts[i].name == 'money' then
			...
			elseif ESX.PlayerData.accounts[i].name == 'vip_money' then
                vipmoney = ESX.PlayerData.accounts[i].money
          end
        end
		...
		SendNUIMessage({ action = 'setMoney', id = 'vip_money', value = vipmoney })
	...
	if Config.ui.showBath then
		showPlayerStatus = (showPlayerStatus+1)

		TriggerEvent('esx_status:getStatus', 'bath', function(status)
			playerStatus['status'][showPlayerStatus] = {
				name = 'bath',
				value = math.floor(100-status.getPercent())
			}
	...
	if Config.ui.showGym then
		showPlayerStatus = (showPlayerStatus+1)

		TriggerEvent('esx_status:getStatus', 'gym', function(status)
			playerStatus['status'][showPlayerStatus] = {
				name = 'gym',
				value = math.floor(100-status.getPercent())
			}
